...
## The browser must never import `@solana/web3.js`

Those packages are API-only and lazily imported (`apps/api/src/chain/index.ts`), so the
default simulated setup never loads them. The client signs opaque bytes with
`@noble/curves/ed25519`. If you find yourself adding a Node polyfill to the Vite config, the
boundary has been crossed somewhere — find it instead.

`npm run build` should keep showing zero matches for `solana` in `apps/web/dist/assets/*.js`.

## The server composes transactions; the client only signs

Never accept client-composed bytes for the relayer to co-sign. `submitTransfer` takes the
server's own `TransferRequest` and must prove the bytes say exactly that. See SECURITY.md.
...
serves both. Adding a third backend should not touch the web app.
...
`node --test` via `tsx`, no framework. `apps/api/src/testkit.ts` gives you an in-memory
